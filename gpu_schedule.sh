...
# Get raw sinfo output and parse properly
sinfo -NO "%n %G %G" -p "$PARTITION" 2>/dev/null | tail -n +2 | while read -r line; do
    node=$(echo "$line" | awk '{print $1}')
    gres=$(echo "$line" | awk '{print $2}')
    gres_used=$(echo "$line" | awk '{print $3}')
    
    [[ -z "$node" ]] && continue
    
    # Parse total GPUs from GRES column (: gpu:hformat200:8 or gpu:8)
    total=0
    if echo "$gres" | grep -q "gpu:h200:"; then
        total=$(echo "$gres" | grep -oP 'gpu:h200:\K[0-9]+')
    elif echo "$gres" | grep -q "gpu:"; then
        total=$(echo "$gres" | grep -oP 'gpu:\K[0-9]+')
    else
        continue
    fi
    
    # Parse used GPUs from GRES_USED column (format: gpu:h200:8(IDX:0-7) or gpu:h200:0(IDX:N/A))
    used=0
    if echo "$gres_used" | grep -q "IDX:"; then
        idx=$(echo "$gres_used" | grep -oP 'IDX:\K[0-9,\-]+')
        if [[ "$idx" != "N/A" ]] && [[ -n "$idx" ]]; then
            used=$(echo "$idx" | tr ',' '\n' | while read -r r; do
                if [[ "$r" == *-* ]]; then
